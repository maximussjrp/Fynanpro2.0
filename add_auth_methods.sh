...
# 1. Adicionar métodos forgotPassword e resetPassword ao auth.service.ts
cat >> /opt/utop/backend/src/services/auth.service.ts << 'EOFAUTH'
...
echo "✅ Métodos adicionados ao auth.service.ts"

# 2. Fechar a classe corretamente
# Precisamos remover a última chave } e adicionar os métodos antes dela
# Na verdade o cat >> adiciona ao final, então precisamos que a classe ainda não esteja fechada
