/**
 * Pricing — fonte única de verdade dos preços oficiais dos planos UTOP.
 *
 * Toda referência a valor de plano no backend deve importar daqui.
 * Frontend espelha em `frontend/src/lib/pricing.ts`.
 *
 * Convenção:
 *   - `*_REAIS` em decimal (R$)
 *   - `*_CENTS` em inteiro de centavos (Asaas/Subscription.amountCents)
 *   - `savingsReais` = (mensal_equivalente × N_meses) − preço_total
 *   - `pricePerMonthReais` = preço_total / N_meses (arredondado a 2 casas)
 */
...
/** Helper: formata em padrão brasileiro "R$ 79,90". */
